...
- **粘り強く検証する**: 精度を改善できそうな仮説・実験があるなら、**少し試して結論を出さない**。パラメータ・特徴量・前処理・CV 設計を変えながら、なんとかして改善できないか**粘り強く**追い込む。粘っても改善しなかった場合は、そこで終わらせず、**予測結果（誤り例・サブ集団別・pred vs GT）を確認しながら深く考察**し、なぜ効かないのかを言語化して次の仮説に繋ぐ。
